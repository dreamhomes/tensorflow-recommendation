    ...
    for epoch_i in range(num_epochs):
        ...
        for batch_i in range(len(train_X) // batch_size):
            ...
            step, train_loss, summaries, _ = sess.run(
                [global_step, loss, train_summary_op, train_op], feed)  # cost
            losses['train'].append(train_loss)
            train_summary_writer.add_summary(summaries, step)  #

            # Show every <show_every_n_batches> batches
            if (epoch_i * (len(train_X) // batch_size) +
        ...
        for batch_i in range(len(test_X) // batch_size):
            ...
            step, test_loss, summaries = sess.run(
                [global_step, loss, inference_summary_op], feed)  # cost
            ...
            time_str = datetime.datetime.now().isoformat()
            if (epoch_i * (len(test_X) // batch_size) +
                    batch_i) % show_every_n_batches == 0:
